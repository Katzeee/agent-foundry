...
Each map lives at `<tracker-root>/<map>/map.md`; its child tickets live at `<tracker-root>/<map>/issues/NN-<type>-<slug>.md`. The filename Type must match the ticket's `Type:` field, so the ticket's kind stays visible in directory listings. A ticket's number is unique within its map and is its identity.
