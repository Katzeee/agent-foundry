...
**Where the map, its child tickets, blocking, and frontier queries physically live is tracker-specific.** Ensure `docs/agents/issue-tracker.md` is usable — run `/wayfinder-setup` if not — and consult its "Wayfinding operations" section. Before interpreting the user's request, collect context.
...
Each ticket is a **child issue** of the map; the tracker's issue id is its identity. Its question is sized to one 100K token agent session:
...
The answer is recorded when the ticket closes (see [Work through the map](#work-through-the-map)). Assets created while resolving a ticket are linked from the issue, not pasted in.
...
4. **Create a child ticket for each question you can specify now** — then wire blocking edges in a **second pass** (issues need ids before they can reference each other). Wiring sorts them into the frontier and the blocked; everything you can't yet specify stays in the fog — the **Not yet specified** section.
