...
FIX carries each confirmed lead to its **honest boundary**. A change that leaves the same ownership problem standing behind a comment, carve-out, or stopgap — or that reports a still-leaking lead as resolved — is a **patch**, and a patch is not a fix. When honest closure needs a bigger surface than first guessed, that surface *is* the work; follow it without mistaking a larger file count for scope creep. The rule is not "finish everything now" — it is: **close honestly, or mark blocked; never disguise the unclosed as fixed.**
...
A `keep` may not leave a known inconsistency standing behind a comment: if a truth is enforced unevenly across its inlets (a guard on some, a comment excusing the rest), that is `extract`/unify — route every inlet through the one owner — not `keep`.
...
1. **Cover every catalog signal** by a method fit for it — grep for textual signals; static enumeration or call-graph for structural ones (fake seam, god-funnel, mixed id-spaces). Record method and coverage per signal class, even at zero hits.
2. **Cluster into leads** — group hits by a shared suspected truth/owner/pattern; dismiss obvious non-candidates with a local, observable reason; uncertain cases become leads. Sketch ownership where it helps reveal a truth owned twice — owners stay tentative.
3. **Record each lead** — the observable reason it deserves inspection, never a verdict.

Lead table:
```
| ID | Location | Signal | Suspicion | Why inspect |
```
- **Signal** — `A1|A2|A3|B1|B2|B3|B4|unused` from the catalog.
...
**Completion:** every catalog signal checked across the declared scope; every hit accounted for by a lead, a grouped pattern, or a locally justified dismissal; every lead names the observable reason it deserves inspection; no verdicts forced; no file modified.
...
| B3 God-funnel | one chokepoint serving flows that change for different reasons; branches *inside* to tell them apart | Does the funnel own a real shared invariant, or just route flows that change for different reasons? |
